...
- **`.layout` (sidebar+content) ไม่มี max-width แล้ว** — เต็มความกว้างจอเสมอ (`width:100%`, padding ซ้ายขวา 40px) ตามที่ผู้ใช้ยืนยันชัดเจนว่าไม่ต้องการขอบเขตตายตัว ไม่ว่าจอจะกว้างแค่ไหน (ลอง 1200px → 1400px ก่อนแล้วยังไม่พอ ผู้ใช้เลยขอเอา cap ออกไปเลย) — **อย่าใส่ max-width กลับเข้าไปใน `.layout` โดยไม่ถาม**
