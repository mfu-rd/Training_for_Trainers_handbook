...
- **`.layout` (sidebar+content) ไม่มี max-width แล้ว** — เต็มความกว้างจอเสมอ (`width:100%`, padding ซ้ายขวา **64px บน desktop / 20px บนมือถือ** ผ่าน media query override) ตามที่ผู้ใช้ยืนยันชัดเจนว่าไม่ต้องการขอบเขตตายตัว ไม่ว่าจอจะกว้างแค่ไหน (ลอง 1200px → 1400px ก่อนแล้วยังไม่พอ ผู้ใช้เลยขอเอา cap ออกไปเลย จากนั้นขอเพิ่ม padding ข้างจาก 40px เป็น 64px อีกรอบ) — **อย่าใส่ max-width กลับเข้าไปใน `.layout` โดยไม่ถาม**
- `.wrap` (hero/footer, มือถือ nav-inner) ยังใช้ `--container-max:1400px` ตามเดิม เพราะไม่มีใครติงเรื่องพื้นที่ว่างในส่วนนี้ (hero มีพื้นหลัง gradient เต็มจออยู่แล้วไม่มีปัญหาที่ว่าง)
